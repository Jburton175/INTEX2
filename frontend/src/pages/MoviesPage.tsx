...
const getGenresFromQuery = (): string[] => {
  const params = new URLSearchParams(window.location.search);
  return params.get("genres")?.split(",") || [];
};
    ...
    async (page: number) => {
      try {
        ...
        if (searchQuery.trim()) {
          ...
          url = `${API_BASE}/INTEX/SearchMovieTitles?${searchParams.toString()}`;
        } else if (selectedGenres.length > 0) {
          searchParams.set("genres", selectedGenres.join(","));
          url = `${API_BASE}/INTEX/GetFilteredMovies?page=${page}&pageSize=${PAGE_SIZE}&${searchParams.toString()}`;
        } else {
          url = `${API_BASE}/INTEX/GetAllMovies?page=${page}&pageSize=${PAGE_SIZE}`;
        }
  ...
  return (
    <div className={styles.moviesPage}>
      ...
      <div className={styles.mainContent}>
        ...
        {!searchQuery && <div ref={sentinelRef} className={styles.sentinel} />}
        {loading && (
          <div className={styles.loading}>
            <div className={styles.spinner} />
            Loading {searchQuery ? "results" : "more movies"}...
          </div>
        )}
      </div>
      <Footer />
